...
/* USER CODE BEGIN PV */
static uint32_t cdc_last_tick;
static uint32_t cdc_counter;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    if ((HAL_GetTick() - cdc_last_tick) >= 1000U)
    {
      char msg[48];
      int len;

      cdc_last_tick = HAL_GetTick();
      len = snprintf(msg, sizeof(msg), "CDC alive %lu\r\n", (unsigned long)cdc_counter++);
      if (len > 0)
      {
        (void)CDC_Transmit_HS((uint8_t *)msg, (uint16_t)len);
      }
    }
  }
  /* USER CODE END 3 */
