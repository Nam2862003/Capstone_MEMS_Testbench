...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define Max_ADC_BUFFER_SIZE 32768 // 128kB buffer, can hold 16384 samples of 2 ADCs (32 bits each)
...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef* hadc)
{
  ...
  {
    // Send the first half of the buffer
    send_adc_data(&adc_buffer[0], Max_ADC_BUFFER_SIZE / 2);
  }
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
  if(hadc->Instance == ADC1)
  {
    // Send the second half of the buffer
    send_adc_data(&adc_buffer[Max_ADC_BUFFER_SIZE / 2], Max_ADC_BUFFER_SIZE / 2);
  }
}
...
void send_adc_data(uint32_t *data, uint32_t length)
{
    ...
    // Clean Cache so USB sees the RAM data
    SCB_CleanDCache_by_Addr((uint32_t*)data, length * 4);
